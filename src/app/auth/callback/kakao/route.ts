...
export async function GET(request: NextRequest) {
  ...
  const loginFailedUrl = new URL(`${PAGE_PATHS.AUTH_LOGIN}?error=kakao_login_failed`, request.url)
  const authFailedUrl = new URL(`${PAGE_PATHS.AUTH_LOGIN}?error=kakao_auth_failed`, request.url)

  if (error || !code) {
    return NextResponse.redirect(authFailedUrl)
  }

  const { data, error: apiError } = await authRepository.kakaoLogin({ code })
