...
interface KakaoLinkSectionProps {
  accessToken: string
}

export default function KakaoLinkSection({ accessToken }: KakaoLinkSectionProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startProcessing(async () => {
      const result = await kakaoLinkAccountAction(accessToken, phoneVerifyToken)
  ...
  return (
    <section className="min-h-screen">
      <Header variant="white" height={55}>
        ...
        <HeaderCenter>
          <HeaderTitle>카카오 계정 연동</HeaderTitle>
        </HeaderCenter>
      </Header>
      <form onSubmit={handleSubmit}>
        <div className="px-[15px] py-[30px] flex flex-col gap-5">
          ...
          <AppSubmitButton isSubmitting={isProcessing} loadingText="연동 중">
            카카오 계정 연동하기
          </AppSubmitButton>
        </div>
      </form>
