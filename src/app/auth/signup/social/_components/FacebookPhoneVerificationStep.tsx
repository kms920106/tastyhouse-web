...
import { facebookLinkAccountAction } from '@/actions/auth'
import { PAGE_PATHS } from '@/lib/paths'
import { useRouter } from 'next/navigation'
import type { FacebookProfile } from '@/domains/auth'

interface FacebookPhoneVerificationStepProps {
...
export default function FacebookPhoneVerificationStep({
  facebookTempToken,
  onLinked,
  onNeedsSignUp,
}: FacebookPhoneVerificationStepProps) {
  ...
  const handleVerified = (verifiedPhone: string, phoneVerifyToken: string) => {
    startProcessing(async () => {
      const result = await facebookLinkAccountAction(facebookTempToken, phoneVerifyToken)
      ...
      if (result.status === 'NEEDS_SIGN_UP') {
        onNeedsSignUp({
          facebookTempToken: result.facebookTempToken,
      ...
      toast('페이스북 인증 정보가 없습니다. 다시 로그인해 주세요.')
