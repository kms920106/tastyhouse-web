...
import { facebookSignUpAction } from '@/actions/auth'
import {
...
import type { FacebookProfile } from '@/domains/auth'
...
const facebookSignupSchema = z.object({
  nickname: z
    .string()
...
type FormErrors = Partial<
  Record<keyof z.infer<typeof facebookSignupSchema> | 'birthDate' | 'email', string>
>

interface FacebookSignupSectionProps {
  facebookTempToken: string
  facebookProfile: FacebookProfile
  phone: string
}

export default function FacebookSignupSection({
  facebookTempToken,
  facebookProfile,
  phone,
}: FacebookSignupSectionProps) {
  ...
  useEffect(() => {
    if (facebookProfile.email) emailVerification.setEmail(facebookProfile.email)
    setNickname(facebookProfile.nickname)
    if (facebookProfile.name) setFullName(facebookProfile.name)
  ...
  const validateForm = (): boolean => {
    const result = facebookSignupSchema.safeParse({
      nickname,
      fullName,
      birthYear,
      birthMonth,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startSubmitTransition(async () => {
      const result = await facebookSignUpAction({
        facebookTempToken,
        ...
        gender: gender!,
        birthDate,
        phoneNumber: phone || facebookProfile.phoneNumber || '',
