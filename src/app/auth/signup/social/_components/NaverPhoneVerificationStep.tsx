...
import { naverLinkAccountAction } from '@/actions/auth'
import { PAGE_PATHS } from '@/lib/paths'
import { useRouter } from 'next/navigation'
import type { NaverProfile } from '@/domains/auth'

interface NaverPhoneVerificationStepProps {
...
export default function NaverPhoneVerificationStep({
  naverTempToken,
  onLinked,
  onNeedsSignUp,
}: NaverPhoneVerificationStepProps) {
  ...
  const handleVerified = (verifiedPhone: string, phoneVerifyToken: string) => {
    startProcessing(async () => {
      const result = await naverLinkAccountAction(naverTempToken, phoneVerifyToken)
      ...
      if (result.status === 'NEEDS_SIGN_UP') {
        onNeedsSignUp({
          naverTempToken: result.naverTempToken,
          naverProfile: result.naverProfile!,
          phone: verifiedPhone,
          phoneVerifyToken,
        })
        return
      ...
      toast('네이버 인증 정보가 없습니다. 다시 로그인해 주세요.')
