...
import { naverSignUpAction } from '@/actions/auth'
import {
...
import type { NaverProfile } from '@/domains/auth'
...
const naverSignupSchema = z.object({
  nickname: z
    .string()
...
type FormErrors = Partial<
  Record<keyof z.infer<typeof naverSignupSchema> | 'birthDate' | 'email', string>
>

interface NaverSignupSectionProps {
  naverTempToken: string
  naverProfile: NaverProfile
  phone: string
}

export default function NaverSignupSection({
  naverTempToken,
  naverProfile,
  phone,
}: NaverSignupSectionProps) {
  ...
  useEffect(() => {
    if (naverProfile.email) emailVerification.setEmail(naverProfile.email)
    setNickname(naverProfile.nickname)
    if (naverProfile.name) setFullName(naverProfile.name)
  ...
  const validateForm = (): boolean => {
    const result = naverSignupSchema.safeParse({
      nickname,
      fullName,
      birthYear,
      birthMonth,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startSubmitTransition(async () => {
      const result = await naverSignUpAction({
        naverTempToken,
        ...
        gender: gender!,
        birthDate,
        phoneNumber: phone || naverProfile.phoneNumber || '',
