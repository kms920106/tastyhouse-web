...
import { ProductDetailReviewStatisticSkeleton } from './ProductDetailReviewStatisticSkeleton'

interface Props {
  productId: number
}

export default function ProductDetailReviewStatistic({ productId }: Props) {
  const { data, isLoading, error } = useProductReviewStatistics(productId)

  if (isLoading) {
    return <ProductDetailReviewStatisticSkeleton />
  }
